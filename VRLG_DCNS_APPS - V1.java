...
//			moduleMapping_1.addModuleToDevice("object_tracker", "cloud"); // placing all instances of Object Tracker module in the Cloud
			...
			for(FogDevice device : fogDevices){
				...
				if(device.getName().startsWith("RSFC")){
					moduleMapping_0.addModuleToDevice("ResoureShedule0", device.getName());  // fixing all instances of the Client module to the Smartphones
					moduleMapping_1.addModuleToDevice("ResoureShedule1", device.getName());  // fixing all instances of the Client module to the Smartphones
				}
				...
				if(device.getName().startsWith("d")){
					moduleMapping_0.addModuleToDevice("concentration_calculator", device.getName());  // fixing all instances of the Client module to the Smartphones
					moduleMapping_1.addModuleToDevice("object_detector", device.getName());  // fixing all instances of the Client module to the Smartphones
					moduleMapping_1.addModuleToDevice("object_tracker", device.getName());  // fixing all instances of the Client module to the Smartphones
	...
	private static Application createApplication0(String appId, int userId){
		 ...
		 */
		application.addAppModule("ResoureShedule0", 10);
		...
		application.addAppEdge("client", "ResoureShedule0", 100, 100, "_SENSOR1", Tuple.UP, AppEdge.MODULE); // adding edge from Client to Concentration Calculator module carrying tuples of type _SENSOR
		application.addAppEdge("ResoureShedule0", "concentration_calculator", 3500, 500, "_SENSOR2", Tuple.DOWN, AppEdge.MODULE); // adding edge from Client to Concentration Calculator module carrying tuples of type _SENSOR
		...
		application.addAppEdge("concentration_calculator", "ResoureShedule0", 3500, 500, "CONCENTRATION1", Tuple.UP, AppEdge.MODULE); // adding edge from Client to Concentration Calculator module carrying tuples of type _SENSOR
		application.addAppEdge("ResoureShedule0", "client", 14, 500, "CONCENTRATION", Tuple.DOWN, AppEdge.MODULE);  // adding edge from Concentration Calculator to Client module carrying tuples of type CONCENTRATION
		 ...
		 */
		application.addTupleMapping("client", "EEG", "_SENSOR1", new FractionalSelectivity(0.9)); // 0.9 tuples of type _SENSOR are emitted by Client module per incoming tuple of type EEG 
		...
		application.addTupleMapping("ResoureShedule0", "_SENSOR1", "_SENSOR2", new FractionalSelectivity(1.0)); // 1.0 tuples of type SELF_STATE_UPDATE are emitted by Client module per incoming tuple of type CONCENTRATION 
		application.addTupleMapping("ResoureShedule0", "CONCENTRATION1", "CONCENTRATION", new FractionalSelectivity(1.0)); // 1.0 tuples of type GLOBAL_STATE_UPDATE are emitted by Client module per incoming tuple of type GLOBAL_GAME_STATE 

		application.addTupleMapping("concentration_calculator", "_SENSOR2", "CONCENTRATION1", new FractionalSelectivity(1.0)); // 1.0 tuples of type CONCENTRATION are emitted by Concentration Calculator module per incoming tuple of type _SENSOR 
		application.addTupleMapping("concentration_calculator", "_SENSOR2", "PLAYER_GAME_STATE", new FractionalSelectivity(1.0)); // 1.0 tuples of type GLOBAL_STATE_UPDATE are emitted by Client module per incoming tuple of type GLOBAL_GAME_STATE 
		 ...
		 */
		final AppLoop loop1 = new AppLoop(new ArrayList<String>(){{add("EEG");add("client");add("ResoureShedule0");add("concentration_calculator");add("ResoureShedule0");add("client");add("DISPLAY");}});
	...
	private static Application createApplication1(String appId, int userId){
		 ...
		 * Adding modules (vertices) to the application model (directed graph)
		 */
		application.addAppModule("ResoureShedule1", 20);
		...
		application.addAppEdge("motion_detector", "ResoureShedule1", 1000, 500, "_SENSOR_1", Tuple.UP, AppEdge.MODULE); // adding edge from Client to Concentration Calculator module carrying tuples of type _SENSOR
		
		application.addAppEdge("ResoureShedule1", "object_detector", 1000, 500, "_SENSOR_2", Tuple.DOWN, AppEdge.MODULE); // adding edge from Client to Concentration Calculator module carrying tuples of type _SENSOR
		...
		application.addAppEdge("object_tracker", "user_interface", 14, 500, "DISPLAY1", Tuple.UP, AppEdge.MODULE);  // adding edge from Concentration Calculator to Client module carrying tuples of type CONCENTRATION

		application.addAppEdge("object_tracker", "ResoureShedule1", 100, 500, "OBJID2", Tuple.UP, AppEdge.MODULE);  // adding edge from Client module to Display (actuator) carrying tuples of type SELF_STATE_UPDATE
		application.addAppEdge("ResoureShedule1", "PTZ_CONTROL", 100, 500, "PTZ_PARAM", Tuple.DOWN, AppEdge.ACTUATOR);  // adding edge from Client module to Display (actuator) carrying tuples of type SELF_STATE_UPDATE
		...
		application.addTupleMapping("ResoureShedule1", "_SENSOR_1", "_SENSOR_2", new FractionalSelectivity(1.0)); // 1.0 tuples of type SELF_STATE_UPDATE are emitted by Client module per incoming tuple of type CONCENTRATION 
		application.addTupleMapping("ResoureShedule1", "OBJID2", "PTZ_PARAM", new FractionalSelectivity(1.0)); // 1.0 tuples of type GLOBAL_STATE_UPDATE are emitted by Client module per incoming tuple of type GLOBAL_GAME_STATE 

		application.addTupleMapping("object_detector", "_SENSOR_2", "OBJID", new FractionalSelectivity(1.0)); // 1.0 tuples of type SELF_STATE_UPDATE are emitted by Client module per incoming tuple of type CONCENTRATION 
		application.addTupleMapping("object_tracker", "OBJID", "OBJID2", new FractionalSelectivity(1.0)); // 1.0 tuples of type CONCENTRATION are emitted by Concentration Calculator module per incoming tuple of type _SENSOR 

		application.addTupleMapping("user_interface", "OBJID2", "GLOBAL_DISPLY", new FractionalSelectivity(1.0)); // 1.0 tuples of type GLOBAL_STATE_UPDATE are emitted by Client module per incoming tuple of type GLOBAL_GAME_STATE 
	
		/*
		 * Defining application loops to monitor the latency of. 
		 * Here, we add only one loop for monitoring : EEG(sensor) -> Client -> Concentration Calculator -> Client -> DISPLAY (actuator)
		 */
		final AppLoop loop1 = new AppLoop(new ArrayList<String>(){{add("CAMERA");add("motion_detector");add("ResoureShedule1");add("object_detector");add("object_tracker");add("ResoureShedule1");add("PTZ_CONTROL");}});
		List<AppLoop> loops = new ArrayList<AppLoop>(){{add(loop1);}};
		application.setLoops(loops);
